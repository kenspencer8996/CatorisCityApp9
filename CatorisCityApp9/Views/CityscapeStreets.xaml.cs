using CatorisCityAppNew.Controllers;
...
using CatorisCityAppNew.Views.Controls.Police;
using CityAppServices;
using CityAppServices.Objects;
using CityAppServices.Objects.Entities;
using System.Collections.Immutable;
using PoliceStationContent = CatorisCityAppNew.Views.Controls.Police.PoliceStationContent;

namespace CatorisCityAppNew.Views;

public partial class CityscapeStreets : ContentView
{
    CityscapeStreetsController _controller;
    int badcount = 0;
    int tentx = 550;
    int tenty = 230;
    Image tentImage = new Image();
    ...
    {
        BadPersonContent badguy = (BadPersonContent)sender;
        ...
        try
        {
            ...
            if (int.IsEvenInteger(GotoCampfire))
            {
                ...
                PersonViewModel badGuyWithPosition = new PersonViewModel();
                badGuyWithPosition.BadPerson = badguy;
                badGuyWithPosition.CampfireLocation = GetCanmpFireLocation(badguy.BadGuy.PersonName);
    ...
    private void MoveBadGuyToCampfire(PersonViewModel badGuyWithPosition)
    {
        int travelIndex = -1;
        try
        {
    ...
    private void MoveCarXOnly(double startx, double x, double y,
            BadPersonContent badguy, int index)
    {
        try
        {
            Global.WriteDebugOutput(" startx " + startx + " x " + x + " y " + y + " badguyname " + badguy.BadGuy.PersonName);
    ...
    private void MoveBadGuy(BadPersonContent badguy,double x, double v)
    {
        Rect rect = new Rect();
        rect.X = x;
        rect.Y = v;
    ...
    public void MoveBadGuyToCampfireApproach(
        PersonViewModel badGuyWithPosition,
    ...
    private void StopBadGuyandMovetoTent(BadPersonContent badguy)
    {
    ...
    internal void AddBadguy(PersonViewModel person)
    {
    ...
    private void ToolbarSelector_Clicked(object sender, EventArgs e)
    {
        ...
        else
        {
           ...
           // GlobalServices.UpdateUISettings();

            GlobalServices.InsertSetting("campfire", "", Global.CampFire);
            GlobalServices.InsertSetting("travelspeed", "", Global.TravelSpeed);
            GlobalServices.InsertSetting("badguycount", "", Global.BadguyCount);
            GlobalServices.InsertSetting("policecarspeed", "", Global.PoliceCarSpeed);
            GlobalServices.InsertSetting("campfiresleeptime", "", Global.CampFireSleepTime);
            ResetTimers();
        }
    }
}