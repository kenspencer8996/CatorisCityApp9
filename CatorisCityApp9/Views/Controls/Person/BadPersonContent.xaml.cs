...
public partial class BadPersonContent : ContentView
{
    public event EventHandler<BadPersonTimerFiredEventArg> ShowBadGuyTimerFired;
    ...
    {
        BadPersonDroppedFiredEventArg args = new BadPersonDroppedFiredEventArg(_personNumber, BadGuy);
        if (BadPersonDroppedEvent != null)
            BadPersonDroppedEvent(this, args);
    }
    public void SetPerson(PersonViewModel badperson, int personNumber)
    {
        try
        {
            BadGuy = badperson;
            ...
            ShowBadGuyTimerFired += Person_PersonTimerFired;
            SetupandStartTimers();
    ...
    private void SetupandStartTimers()
    {
        ...
        _badGuyMoveTimer.Tick += (sender, e) => BadGuyMove_TimerFired();
        ...
        _badGuyEscapeMoveTimer.Tick += (sender, e) => BadGuyEsacpeMove_TimerFired();
        ResetTimer();
    }
    ...
    private void BadGuyEsacpeMove_TimerFired()
    {
    ...
    private void BadGuyMove_TimerFired()
    {
        _badGuyMoveTimer.Stop();
        if (_TimerFired == false)
        {
            BadPersonMoveFrmTentEvent(this, new BadPersonMoveFiredEventArg(0, 0, BadGuy));
            _TimerFired = true;
        }
    }
    ...
    private void BadGuyEsacpeMoveTimer_TimerFired()
    {
        _badGuyEscapeMoveTimer?.Stop();
        MoveBadHuyOnEscapePath();
        _iterationForEscapePaths++;
    ...
    private void BadGuyMoveTimer_TimerFired()
    {
        _badGuyMoveTimer.Stop();
        if (_TimerFired == false)
        {
    ...
    {
        WeakReferenceMessenger.Default.Send(new PersonMessage(BadGuy.Name));
    }
    ...
    private void MoveBadGuyFromAnimation(double x, double y, string info, bool onlyYNeg)
    {
        try
        {
            ...
            BadPersonMoveFiredEventArg arg = new BadPersonMoveFiredEventArg(x, y,BadGuy);
            if (BadPersonMoveEvent != null)
            {
                BadPersonMoveEvent(this, arg);
            }
    ...
    private void OnDragStartingMyPerson(object sender, DragStartingEventArgs e)
    {
        ...
        e.Data.Properties.Add("BadPerson", BadGuy);
    }
    private void SetImage(PersonImageStatusEnum status)
    {
        var foundImages = from i in BadGuy.Images
    ...
    internal void StartMainAnimation()
    {
        var found = from d in Global.CampfireSpots
                    where d.RobberName == BadGuy.Name
                    select d;
