        ...
        private  void AddLabels(string streetName, int x, int y, int StreetLength)
        {
            ...
            switch (streetName)
            {
                case "You St":
                    ...
                    Global.AddYouStreetLabel(StreetLength /2, yyou);
                    break;
                case "Moo Dr":
                    int xmoo = x  + Global.StreetWidth + 140;
                    int ymoo = (StreetLength / 2) + 50;
                    Global.AddMooDrLaneLabel(xmoo, ymoo);
                    break;
                case "Tea St":
                    int xtea = x +225;
                    int ytea = y + 100;
                    Global.AddTeaStreetLabel(xtea, ytea);
                    break;
                case "Yodel Lane":
                    int xyodel = 70; // + Global.LotSizeNormal - 20;
                    Global.AddYodelLaneLabel(xyodel, 220);
                    break; 
                case "Mik Ave":
                    int xmik = x +(StreetLength/2);
                    int miky = y + Global.StreetWidth / 2 + Global.LotSizeNormal + 20;
                    Global.AddMikStreetLabel(xmik, miky);
                    break;
            }
        }
    }
