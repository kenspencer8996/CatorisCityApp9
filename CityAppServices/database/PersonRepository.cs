    ...
    public class PersonRepository
    {
        ...
        public async Task<PersonEntity> UpsertPerson(PersonEntity person)
        {
            ...
            else
            {
                ...
                {
                    string role = person.PersonRole.ToString();
                    string sqlraw = "INSERT INTO person ( Name,IsUser,Funds,PersonRole,CurrentImageKey,CurrentImage,Active) ";
                    sqlraw+= "VALUES('" + person.Name + "'," + isuser + ",0,'" + role + "'," + person.CurrentImageKey + ",'" + person.CurrentImage + "'," + person.Active + ")"; 
                    var command = new SqliteCommand();
                    command.Connection = connection;
                    command.CommandText = sqlraw;
                    command.ExecuteNonQuery();
